...
class OccupancyGridNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('inflation_radius', 0.18) # meters
        ...
        #     Odometry, '/map', self.map_callback, 10
        # )
        self.map_sub = self.create_subscription(
        ...
        # Timer to publish map periodically (disabled: inflate directly from /map callback)
    ...
    def timer_callback(self):
        ...
        # Disabled for /map -> /inflated_map mode.
        # Keep this logic for reference if you switch back to scan-based mapping.
        return
    ...
    def map_callback(self, msg):
        """Inflate incoming /map OccupancyGrid and publish /inflated_map."""
        # --- Old occupancy-map generation path kept for reference ---
        ...
        if width == 0 or height == 0:
            self.get_logger().warn('Received empty /map, skipping inflation.')
            return

        map_array = np.array(msg.data, dtype=np.int16).reshape((height, width))
        # Adapt incoming OccupancyGrid data into GridMap internals,
        # then reuse GridMap.get_inflated_grid without changing its logic.
        grid_log_odds = np.zeros((height, width), dtype=np.float32)
        grid_log_odds[map_array == 100] = GridMap.LMAX
        grid_log_odds[map_array == 0] = GridMap.LMIN

        self.grid_map.grid = grid_log_odds
        self.grid_map.height = height
        self.grid_map.width = width
        self.grid_map.cell_size = msg.info.resolution

        inflated_array = self.grid_map.get_inflated_grid(self.inflation_radius)

        inflated_msg = OccupancyGrid()
        inflated_msg.header = msg.header
        inflated_msg.info = msg.info
        inflated_msg.data = inflated_array.flatten().astype(np.int8).tolist()
