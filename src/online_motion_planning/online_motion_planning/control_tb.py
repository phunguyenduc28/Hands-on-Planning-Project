...
class DWATurtlebot(Node):
    ...
    def obstacle_cost(self, traj):
        ...
        for x, y, _ in traj:
            ...
            if val > 0:
                # val is in [1, 50]; normalize to [0,1] then square
                normalized = val / 50.0
                penalty += normalized * normalized

            if val == -1:
                # unknown — small penalty, don't want to barrel into the unknown
                penalty += 0.05
    ...
    def compute_dwa(self):
        ...
        # Initial distance to goal (for normalization)
        ...
        # Allow w sweep when v is small, so robot can rotate
        # out of trouble instead of just stopping.
        # Make sure we always include v=0 with rotation options.
        ...
        for v in v_samples:
            for w in np.arange(w_min, w_max + 0.01, 0.06):
                ...
                # Normalize distance by initial distance — prevents far goals
                # from making this term dominate every other consideration.
                dist_norm = dist / init_dist

                # Clearance term: how close was the closest point to obstacle?
                # max_val is in 0..50. Higher = closer to obstacle = worse.
                clearance_cost = (max_val / 50.0)
        ...
        # if everything was rejected, rotate in place to look around
        if best_cost == float('inf'):
            self.get_logger().warn("All trajectories rejected — rotating to escape")
            best_v = 0.0
            best_w = self.max_yaw_rate * 0.5
